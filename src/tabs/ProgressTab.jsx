  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // STRICT FILTER (same as local stats)
...
export default function ProgressTab({ db, onOpenWorkout }) {
  ...
  const { overviewRows, perExerciseSeries } = useMemo(() => {
    // Build time series per exercise: date -> session best set weight for that exercise
    const seriesMap = new Map(); // name -> [{ date, weight }]
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            ...
            <p className="text-xs text-gray-500">
              Bars show Start vs Current; label shows Δ (diff)
            </p>
          </div>
      ...
      {perExerciseSeries.length > 0 && (
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          <h3 className="mb-3 text-base font-semibold">
            Per-Exercise History (Best Set Weight per Session)
          </h3>
